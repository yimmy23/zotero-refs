...
/**
 * Compact card for a citation cluster ("[6-10]"): every entry rendered
 * from LOCAL fused data only — no network requests on hover. Clicking an
 * entry's title opens its full multi-source card at the same anchor.
 */
export function showRefListPopup(
  refs: RefItem[],
  rect: PopupRect,
  actions?: {
    onImport?: (ref: RefItem) => void;
    onExpand?: (ref: RefItem) => void;
  },
): PopupCard {
  const popup = new PopupCard();
  popup.onInit(rect, "top center");
  currentPopup = popup;
  const tagDefaultColor = "#59C1BD";
  popup.addList(
    refs.map((ref) => {
      const tags: RefTag[] = (ref.tags || []).map((tag) =>
        typeof tag === "object"
      ...
      // agency documents (FDA / EMA labels…) often carry only a URL —
      // the native popup links it, so must we
      if (!ids.DOI && ref.url && isHttpUrl(ref.url)) {
        tags.push({ text: "URL", tip: ref.url, url: ref.url });
      }
      if (ref.oaUrl) {
        tags.push({
          text: "PDF",
      ...
      if (ref.libItemID) {
        ...
        tags.push({
          ...
          onClick: () => actions.onImport?.(ref),
        });
      }
      const authors = ref.authors || [];
      const who =
        authors.slice(0, 3).join(", ") + (authors.length > 3 ? " et al." : "");
      const meta = [who, ref.primaryVenue, String(ref.year || "")]
        .map((s) => (s || "").trim())
        .filter(Boolean)
        .join(" · ");
      const body =
        htmlToText(ref.title || "").trim() ||
        collapseText(ref.text || "", 160) ||
        getString("popup-untitled");
      return {
        title: (ref.number ? `${ref.number}. ` : "") + body,
        titleTip: actions?.onExpand
          ? getString("popup-list-expand-tip")
          : undefined,
        meta,
        tags,
        onOpen: actions?.onExpand ? () => actions.onExpand?.(ref) : undefined,
      };
    }),
  );
  return popup;
