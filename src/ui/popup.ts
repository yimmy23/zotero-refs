...
export class PopupCard {
  ...
  /**
   * Fill the card with a compact list of references (citation cluster).
   * Local data only — the option-dot row is hidden and no source lookups
   * run; clicking an entry's title hands off to its full card.
   */
  addList(
    entries: Array<{
      title: string;
    ...
    const doc = getDoc();
    const optionContainer = this.container.querySelector(
      "#option-container",
    ) as HTMLElement;
    optionContainer.style.display = "none";
    const children: TagElementProps[] = entries.map((entry, i) => {
      const rows: TagElementProps[] = [
        {
          tag: "span",
          classList: ["title"],
          properties: { innerText: entry.title },
          attributes: entry.titleTip ? { title: entry.titleTip } : {},
          styles: {
            display: "block",
            fontWeight: "bold",
            fontSize: "1.05em",
            lineHeight: "1.4em",
            color: this.resolveTitleColor(),
            cursor: entry.onOpen ? "pointer" : "text",
          },
          listeners: entry.onOpen
            ? [
                {
                  type: "click",
                  listener: (ev: Event) => {
                    const e = ev as MouseEvent;
                    // leave modified clicks alone (translate / selection)
      ...
      if (entry.meta) {
        rows.push({
          ...
          properties: { innerText: entry.meta },
        });
      }
      if (entry.tags.length) {
        rows.push({
          ...
          classList: ["tags"],
          styles: { width: "100%" },
          children: entry.tags.map((tag) => this.tagChipProps(tag)),
        });
      }
      return {
        tag: "div",
        classList: ["references-popup-entry"],
        styles: {
          padding: "0.35em 0",
          borderTop: i ? "1px solid rgb(128 128 128 / 25%)" : "",
        },
        children: rows,
      };
    });
