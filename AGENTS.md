...
17. **PDF parser: numbering beats geometry.** `mergeSameRef` first tries `mergeNumberedRefs` — an entry starts iff the line begins with the NEXT number in sequence (`numAtStart`: "12." "12)" "[12]" "(12)" "12 Author" and JAMA's "12 . Author"; bare numbers must be followed by a letter so wrapped volume fragments "41 , 1103" don't count). Trailing matter (licence, "Publisher's note", figure legends) is skipped until numbering resumes (Nature Methods refs 69–83). Numbers set in the margin as their own text run (BMJ Fast Facts: "1" at x=36, entry text at x=51, baselines 0.9 pt apart) are glued to the entry text in `mergeSameLine` — without that the number is its own line and `numAtStart` sees nothing. Kept narrow on purpose (bare 1–3-digit run, text to its right on the same band, entry-like first character): a general band-overlap rule merges the page number into the running head and breaks header/footer removal. Geometry/indent merging is only the fallback for unnumbered lists — and for blocks whose numbering is out of sequence (OCR'd scans: ≤5 entries produced against ≥4× as many numbered starts). The no-heading fallback is gated on publication years: ≥50% of the entries it would return must contain a 19xx/20xx, otherwise it returns nothing — affiliation lists, antibody/statistics tables, search strategies and section-heading blocks are all numbered too, and page 1 is now in the walk. `getRefLines` walks pages backwards **to page index 0** (stopping at 1 lost every single-page letter/abstract and short paper); after the heading page it (a) completes the heading page from committed parts (double-spaced manuscripts split every entry into a part), (b) appends later pages whose union of lines picks up the sequence, (c) ignores false headings ("References (160–200)" pointers, blocks that are not reference-like) and the "skip until bottom-right line" heuristic when the running footer leads the content stream. Regression: `scripts/parser-corpus/` (32 journals, 24/27 exact vs Crossref; KEYNOTE-189 → 31, STTT → 79, BMJ → 123, Chest manuscript → 10).
