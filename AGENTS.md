...
Contract for any change: `npm run build` must pass (esbuild + `tsc --noEmit`), the plugin must load in a **Zotero 7, 8 and 9** main window without a single uncaught exception, and the four item-pane sections (References / Cited By / Related / Citation Graph) must render for a normal journal article. Never test against the user's real Zotero profile — always the isolated dev profile (below).
