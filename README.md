...
- **Refresh button**: click to refresh (cache-first); **long-press** to bypass both saved-source and HTTP caches and re-fetch both sides; `Ctrl+click` in a thesis/book PDF parses backwards from the current page (thesis mode). New row edits retain their original citation evidence: refresh reapplies them only to a unique match, otherwise it keeps the edited list and explains why. Clearing saved reference data also clears these edits.
- **Progress and partial results**: available API results appear while PDF extraction continues. A pending or incomplete list is labelled and read-only; copy, details and import remain available. It is not saved as a complete reference list. Online retrieval and PDF extraction share a 30-second interaction budget, including metadata spot checks.
- **Keyboard details**: use the row’s Details button or `Alt+Enter` on its label. `Esc` closes the card and restores focus to its trigger.
...
**参考文献**：自动加载，单一**融合**列表——PDF 文本层提供骨架（论文实际印出的条目、顺序、编号、页内位置），Crossref → Semantic Scholar → OpenAlex → 知网 逐条补全元数据（尤其是 DOI；历史 699 篇语料的 DOI 覆盖率约 19% → 80%，并非本版准确率）。任一侧不可用（未打开 PDF / 无标识符 / 离线）另一侧独立成表；计数旁标注参与的来源，仅在线找到的条目带 `API` 标签追加在表尾。刷新按钮：单击刷新（优先缓存），**长按**跳过已保存来源及 HTTP 缓存重新获取，学位论文中 `Ctrl+单击` 从当前页向前解析。行内操作：实心行=已入库、半透明=未入库；单击**复制干净引文**（自动去掉序号）；长按**编辑**原文（`Esc` 取消）；`Ctrl+单击` 在文库定位或浏览器打开；`+` **导入并双向关联**（`Ctrl+单击 +` 选择目标分类）；`−` 取消关联；所有控件悬停均有提示。被 OpenAlex / PubMed 标记**撤稿**的文献显示红色「已撤稿」标签，导入前会再次确认。工具栏保留原图标并显示操作名称；“更多”菜单提供跳过缓存重新获取、从 PDF 当前页解析及纯文本／Markdown／CSV 导出。筛选支持题录、作者、标识符和徽章，并显示结果数量；批量导入遵循同一筛选条件，执行前确认，中途点击进度窗口即可停止。Enter 复制、Ctrl/Cmd+Enter 定位、F2 编辑、Alt+Enter 或“详情”按钮打开卡片；Esc 关闭卡片并返回原控件；双击计数复制全部。条目按 DOI 查询，无 DOI 时用 Extra/URL 中的 PMID 或 arXiv 号，PubMed 导入的条目同样可用。
...
**刷新与编辑保护**：本版起的行编辑会保存原引用证据；刷新仅在唯一匹配时套用编辑，无法唯一匹配时保留整个编辑列表并提示。旧版未记录来源的编辑无法自动识别或自动套用；首次替换前的完整列表会留在缓存文件的 `FUSED_BEFORE_REFRESH` 槽，供恢复使用。清除已保存参考文献数据也会清除这些编辑。

**加载与不完整结果**：API 已就绪时会先显示，PDF 在后台继续解析；待融合或不完整列表会明确标记，暂不允许编辑，仍可复制、查看详情和导入。它们不会写成完整参考文献缓存。单次获取、PDF 读取及融合补查共享 30 秒期限；切走条目会取消旧界面的处理，返回后可重试。
